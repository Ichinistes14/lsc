...
const { addEmploye, modifyEmploye, deleteEmploye, addContrat, deleteContrat, deleteFacture, getEmployesWithMetrics, getContrats, getGagnants, addGagnant, deleteGagnant } = require("../lib/google");
...
router.get("/home", checkConnetion, async (req, res) => {
  try {
    ...
    res.render("gestion/home", { employees: sheetData });
  } catch (error) {
    console.error(error);
    res.sendStatus(500);
  }
...
router.post("/add-employe", checkConnetion, async (req, res) => {
  ...
  try {
    ...
    const id = name.split(" ")[0] + `_${Math.floor(1000 + Math.random() * 9000).toString()}`;
    const user = new User({
      id: id,
      role: "user",
      employe: employe._id
...
router.post("/modify-employe", checkConnetion, async (req, res) => {
  ...
  try {
    await modifyEmploye(id, { name: name, grade: grade, date: date, rib: rib, phone: phone});
    await logAction(`Modification d'un employé: ${name}, ${grade}`, req.cookies.id);
...
router.post("/delete-employe", checkConnetion, async (req, res) => {
  ...
  try {
    ...
    if (!employe.user) {
      await logAction(`Suppresion d'un employé ${await employe.name}`, req.cookies.id);
...
router.post("/delete-user", checkConnetion, async (req, res) => {
  ...
  try {
    ...
    if (await employe) {
      await logAction(`Suppresion d'un employé: ${employe.name}`, req.cookies.id);
    }
